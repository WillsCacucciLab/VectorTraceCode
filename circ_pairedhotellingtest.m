function [pval, F] = circ_pairedhotellingtest(alpha1,alpha2,r1,r2)
...
elseif nargin == 2
    ...
    [r1, r2] = deal( ones(size(alpha1)) );   % TW edit: I made some additions that assume the presence of r ..
elseif nargin == 3
...
% TW edit: Remove Nans - fully remove any samples in which any entry (alpha1, 2 or r) is nan.
%          (previously removed alpha1 nans separate to alpha 2, could have resulted in mismatched samples).
nanInd = isnan(alpha1) | isnan(alpha2) ; 
if meanofmeans
    nanInd = nanInd | isnan(r1) | isnan(r2);
end
...
% TW: this is actually eq. 27.27, in section 27.9 (F-stat for one sample, second order hotelling, null hypothesis = there is no mean direction).
%     Section 27.13 discusses how to calculate second order diffs between paired samples, then refers you back to 27.9 for test stat. 
